  ...
  const register = async (username, email, password) => {
    ...
    try {
      ...
      if (Object.keys(validationErrors).length === 0) {
        const response = await axios.post(`http://netflix-watch-web.vercel.app/api/v1/register`, dataToSend);
        if (response) {
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (Object.keys(validationErrors).length === 0) {
        ...
        `http://netflix-watch-web.vercel.app/api/v1/login`,
          dataToSend
        );
        if (response) {
          ...
          <div className="flex justify-center">
            <div className=" w-full bg-black bg-opacity-70 px-16 py-16 mt-2 rounded-md self-center lg:2/5 lg:max-w-md">
              ...
              <div className="flex flex-row gap-4 mt-8 items-center justify-center">
                <div onClick={()=> {window.location.href = `http://netflix-watch-web.vercel.app/api/v1/auth/google`}} className="w-10 h-10 bg-white rounded-full hover:opacity-80 transition cursor-pointer flex items-center justify-center">
                  <FcGoogle size={30}/>
                </div>
                <div onClick={()=> {window.location.href = `http://netflix-watch-web.vercel.app/api/v1/auth/github`}} className="w-10 h-10 bg-white rounded-full hover:opacity-80 transition cursor-pointer flex items-center justify-center">
                  <FaGithub size={30}/>
                </div>
              </div>
